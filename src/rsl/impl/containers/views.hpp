...
namespace rsl
{
    ...
    public:
        ...
        [[rythe_always_inline]] constexpr array_view(std::initializer_list<value_type> initializerList);

        template <size_type N>
        [[rythe_always_inline]] constexpr array_view(T (&arr)[N]) noexcept;

        [[nodiscard]] [[rythe_always_inline]] constexpr operator array_view<const T, ConstIter>() const noexcept
        ...
        [[nodiscard]] [[rythe_always_inline]] constexpr static array_view from_buffer(T* ptr, size_type count) noexcept;
        template <not_same_as<T> Other>
        [[nodiscard]] [[rythe_always_inline]] constexpr static array_view from_buffer(Other* ptr, size_type count) noexcept
            requires same_as<byte, remove_cvr_t<T>> && (is_const_v<T> || !is_const_v<Other>);
        template <contiguous_iterator It>
        [[rythe_always_inline]] constexpr static array_view from_iterator_pair(It first, It last)
    ...
    [[rythe_always_inline]] constexpr size_type linear_search_and_replace(
            ArrayType& arr, const Comparable& key, const Replacement& replacement, size_type offset = 0, size_type endSearch = npos)
            noexcept;
    ...
    [[rythe_always_inline]] constexpr size_type
            linear_search_sequence(array_view<T, Iter, ConstIter> arr, array_view<C, CIter, CConstIter> key) noexcept;
    ...
    template <
            ...
            contiguous_iterator CConstIter>
    [[rythe_always_inline]] constexpr size_type
            linear_search_collection(array_view<T, Iter, ConstIter> arr, array_view<C, CIter, CConstIter> key) noexcept;
    ...
    [[rythe_always_inline]] constexpr size_type reverse_linear_search_outside_collection(
            array_view<T, Iter, ConstIter> arr, array_view<C, CIter, CConstIter> key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, weakly_equality_comparable_with<T> C>
    [[rythe_always_inline]] constexpr size_type linear_search(array_view<T, Iter, ConstIter> arr, const C& key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, weakly_equality_comparable_with<T> C>
    [[rythe_always_inline]] constexpr size_type linear_search_not_eq(array_view<T, Iter, ConstIter> arr, const C& key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, weakly_equality_comparable_with<T> C>
    [[rythe_always_inline]] constexpr size_type reverse_linear_search(array_view<T, Iter, ConstIter> arr, const C& key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, weakly_equality_comparable_with<T> C>
    [[rythe_always_inline]] constexpr size_type
            reverse_linear_search_not_eq(array_view<T, Iter, ConstIter> arr, const C& key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, typename Func>
    [[rythe_always_inline]] constexpr size_type linear_search_custom(array_view<T, Iter, ConstIter> arr, Func&& comparer) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, typename Func>
    [[rythe_always_inline]] constexpr size_type
            reverse_linear_search_custom(array_view<T, Iter, ConstIter> arr, Func&& comparer) noexcept;
    ...
    template <
            ...
            contiguous_iterator CConstIter>
    [[rythe_always_inline]] constexpr size_type
            linear_count_sequence(array_view<T, Iter, ConstIter> arr, array_view<C, CIter, CConstIter> key) noexcept;
    ...
    template <
            ...
            contiguous_iterator CConstIter>
    [[rythe_always_inline]] constexpr size_type
            linear_count_collection(array_view<T, Iter, ConstIter> arr, array_view<C, CIter, CConstIter> key) noexcept;
    ...
    template <
            ...
            contiguous_iterator CConstIter>
    [[rythe_always_inline]] constexpr size_type
            linear_count_outside_collection(array_view<T, Iter, ConstIter> arr, array_view<C, CIter, CConstIter> key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, weakly_equality_comparable_with<T> C>
    [[rythe_always_inline]] constexpr size_type linear_count(array_view<T, Iter, ConstIter> arr, const C& key) noexcept;

    template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter, weakly_equality_comparable_with<T> C>
    [[rythe_always_inline]] constexpr size_type linear_count_not_eq(array_view<T, Iter, ConstIter> arr, const C& key) noexcept;
    ...
    namespace internal
    {
        ...
        template <typename Iter, bool IsBidirectional = bidirectional_iterator<Iter>>
        struct select_reverse_iterator
        {
            using value = void;
        };

        template <typename Iter>
        struct select_reverse_iterator<Iter, true>
        {
            using value = reverse_iterator<Iter>;
        };
    } // namespace internal

    template <
            weak_input_or_output_iterator Iter,
            weak_input_or_output_iterator ConstIter = internal::select_const_iter<iter_read_t<Iter>, Iter>>
    struct iterator_view
    {
    public:
        using value_type = iter_read_t<Iter>;
        using pointer = iter_pointer_t<Iter>;
        using const_pointer = add_const_t<value_type>*;
        using reference = iter_reference_t<Iter>;
        using const_reference = add_const_t<value_type>&;
        using iterator_type = Iter;
        using const_iterator_type = ConstIter;

        constexpr static bool reverse_iterable = bidirectional_iterator<iterator_type>;

        using reverse_iterator_type = internal::select_reverse_iterator<iterator_type>;
        using const_reverse_iterator_type = internal::select_reverse_iterator<const_iterator_type>;
        ...
        [[nodiscard]] [[rythe_always_inline]] constexpr static iterator_view
                from_string_length(pointer str, value_type terminator = value_type{}) noexcept
