#pragma once
#include "delegate.hpp"

namespace rsl
{
    template <typename ReturnType, typename... ParamTypes>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate()
        : delegate(allocator_context::globalAllocator)
    {}

    template <typename ReturnType, typename... ParamTypes>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate(
        const allocator_storage allocator
    )
        : m_alloc(allocator),
          m_invocation()
    {
    }

    template <typename ReturnType, typename... ParamTypes>
    template <invocable<ReturnType(ParamTypes...)> Functor>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate(const Functor& instance)
        : m_alloc(allocator_context::globalAllocator),
          m_invocation(base::template create_element<Functor>(m_alloc, instance))
    {
    }

    template <typename ReturnType, typename... ParamTypes>
    template <invocable<ReturnType(ParamTypes...)> Functor>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate(
        const allocator_storage allocator, const Functor& instance
    )
        : m_alloc(allocator),
          m_invocation(base::template create_element<Functor>(m_alloc, instance))
    {
    }

    template <typename ReturnType, typename... ParamTypes>
    template <functor Functor>
        requires invocable<Functor, ReturnType(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate(const Functor& instance)
        : m_alloc(allocator_context::globalAllocator),
          m_invocation(base::template create_element<Functor>(m_alloc, instance))
    {
    }

    template <typename ReturnType, typename... ParamTypes>
    template <functor Functor>
        requires invocable<Functor, ReturnType(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate(
        const allocator_storage allocator, const Functor& instance
    )
        : m_alloc(allocator),
          m_invocation(base::template create_element<Functor>(m_alloc, instance))
    {
    }

    template <typename ReturnType, typename... ParamTypes>
    template <typename T, ReturnType (T::*TMethod)(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(T& instance)
    {
        return delegate(base::template create_element<T, TMethod>(nullptr, instance));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <typename T, ReturnType (T::*TMethod)(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(allocator_storage allocator, T& instance)
    {
        return delegate(base::template create_element<T, TMethod>(allocator, instance));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <typename T, ReturnType (T::*TMethod)(ParamTypes...) const>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(const T& instance)
    {
        return delegate(base::template create_element<T, TMethod>(nullptr, instance));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <typename T, ReturnType (T::*TMethod)(ParamTypes...) const>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(allocator_storage allocator, const T& instance)
    {
        return delegate(base::template create_element<T, TMethod>(allocator, instance));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <ReturnType (*TMethod)(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create()
    {
        return delegate(base::template create_element<TMethod>(nullptr));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <ReturnType (*TMethod)(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(allocator_storage allocator)
    {
        return delegate(base::template create_element<TMethod>(allocator));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <functor Functor>
        requires invocable<Functor, ReturnType(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(const Functor& instance)
    {
        return delegate(base::template create_element<Functor>(nullptr, instance));
    }

    template <typename ReturnType, typename... ParamTypes>
    template <functor Functor>
        requires invocable<Functor, ReturnType(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>
    delegate<ReturnType(ParamTypes...)>::create(
        allocator_storage allocator, const Functor& instance
    )
    {
        return delegate(base::template create_element<Functor>(allocator, instance));
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr void
    delegate<ReturnType(ParamTypes...)>::set_allocator(const allocator_storage allocator)
        noexcept(is_nothrow_copy_assignable_v<allocator_storage>)
    {
        m_alloc = allocator;
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr memory_allocator&
    delegate<ReturnType(ParamTypes...)>::get_allocator() noexcept
    {
        return *m_alloc;
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr const memory_allocator&
    delegate<ReturnType(ParamTypes...)>::get_allocator() const noexcept
    {
        return *m_alloc;
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr bool delegate<ReturnType(ParamTypes...)>::is_empty() const noexcept
    {
        return m_invocation.stub == nullptr;
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr void delegate<ReturnType(ParamTypes...)>::clear() noexcept
    {
        m_invocation = invocation_element();
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr bool delegate<ReturnType(ParamTypes...)>::operator==(nullptr_type) const noexcept
    {
        return is_empty();
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr bool delegate<ReturnType(ParamTypes...)>::operator!=(nullptr_type) const noexcept
    {
        return !is_empty();
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr bool delegate<ReturnType(ParamTypes...)>::operator==(
        const multicast_delegate<ReturnType(ParamTypes...)>& other
    ) const noexcept
    {
        return other == (*this);
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr bool delegate<ReturnType(ParamTypes...)>::operator!=(
        const multicast_delegate<ReturnType(ParamTypes...)>& other
    ) const noexcept
    {
        return other != (*this);
    }

    template <typename ReturnType, typename... ParamTypes>
    template <typename T, ReturnType (T::*TMethod)(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>&
    delegate<ReturnType(ParamTypes...)>::assign(T& instance)
    {
        m_invocation = base::template create_element<T, TMethod>(m_alloc, instance);
        return *this;
    }

    template <typename ReturnType, typename... ParamTypes>
    template <typename T, ReturnType (T::*TMethod)(ParamTypes...) const>
    constexpr delegate<ReturnType(ParamTypes...)>&
    delegate<ReturnType(ParamTypes...)>::assign(const T& instance)
    {
        m_invocation = base::template create_element<T, TMethod>(m_alloc, instance);
        return *this;
    }

    template <typename ReturnType, typename... ParamTypes>
    template <ReturnType (*TMethod)(ParamTypes...)>
    constexpr delegate<ReturnType(ParamTypes...)>&
    delegate<ReturnType(ParamTypes...)>::assign()
    {
        m_invocation = base::template create_element<TMethod>(m_alloc);
        return *this;
    }

    template <typename ReturnType, typename... ParamTypes>
    template <invocable<ReturnType(ParamTypes...)> Functor>
    constexpr delegate<ReturnType(ParamTypes...)>&
    delegate<ReturnType(ParamTypes...)>::operator=(const Functor& instance)
    {
        m_invocation = base::template create_element<Functor>(m_alloc, instance);
        return *this;
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr ReturnType delegate<ReturnType(ParamTypes...)>::operator()(ParamTypes... args
    ) const
    {
        return invoke(args...);
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr ReturnType delegate<ReturnType(ParamTypes...)>::invoke(ParamTypes... args) const
    {
        return (*m_invocation.stub)(*m_invocation.object, args...);
    }

    template <typename ReturnType, typename... ParamTypes>
    constexpr delegate<ReturnType(ParamTypes...)>::delegate(
        const allocator_storage allocator, invocation_element&& e
    )
        : m_alloc(allocator),
          m_invocation(e)
    {
    }
} // namespace rsl
