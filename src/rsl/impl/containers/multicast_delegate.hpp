...
namespace rsl
{
    namespace internal
    {
        ...
        {
            using type = dynamic_array<T>;
        };

        template <>
        struct multicast_delegate_invocation_result<void>
        {
            using type = void;
        };
    }
    ...
    private:
        template <typename T>
        using invocation_result_t = typename internal::multicast_delegate_invocation_result<T>::type;
