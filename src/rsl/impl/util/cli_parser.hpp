...
namespace rsl
{
   ...
   class cli_parser
    ...
    inline bool cli_parser::is_number(const string_view arg) const
    {
    ...
    inline string_view cli_parser::operator()(std::initializer_list<char const* const> init_list) const
    {
        ...
        {
            auto optIt = m_params.find(trim_leading_dashes(string_view::from_string_length(name)));
            if (optIt)
            {
                return *optIt;
            }
    ...
    string_view cli_parser::operator()(string_view name, string_view defaultValue) const
    {
        auto optIt = m_params.find(trim_leading_dashes(name));
        if (optIt)
        {
            return *optIt;
        }
    ...
    // same as above but for a list of names. returns the first value to be found.
    string_view cli_parser::operator()(std::initializer_list<char const* const> init_list, string_view defaultValue) const
    {
        for (auto& name : init_list)
        {
            auto optIt = m_params.find(trim_leading_dashes(string_view::from_string_length(name)));
    ...
    string_view cli_parser::operator()(size_type index, string_view defaultValue) const
    {
        if (index < m_posArgs.size())
        {
            return m_posArgs[index];
    ...
    inline multimap_iteration_wrapper cli_parser::params(string_view name) const
    {
        auto trimmed_name = trim_leading_dashes(name);
        return multimap_iteration_wrapper(m_params.lower_bound(trimmed_name), m_params.upper_bound(trimmed_name));
    }
} // namespace rsl
